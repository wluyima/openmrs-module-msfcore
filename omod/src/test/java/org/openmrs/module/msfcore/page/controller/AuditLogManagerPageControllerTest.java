...
public class AuditLogManagerPageControllerTest {
    ...
    public void before() throws ParseException {
        ...
        viewPatient = AuditLog.builder().event(Event.VIEW_PATIENT).detail("viewed patient dashboard").patient(patient).date(
                        DateUtils.parse("2018-07-01", "yyyy-MM-dd")).build();
        registerPatient = AuditLog.builder().event(Event.REGISTER_PATIENT).detail("registered patient").patient(new Patient(2)).date(
                        DateUtils.parse("2018-07-25", "yyyy-MM-dd")).build();
        login = AuditLog.builder().event(Event.LOGIN).detail("user logged in").user(user).date(DateUtils.parse("2018-08-01", "yyyy-MM-dd"))
                        .build();
    }
