...
/**
 *  Adds the form name information to the encounter template since multiple forms are entered
 */
@Component
    ...
    public boolean mapRequest(FragmentRequest request) {
        if (request.getProviderName().equals("coreapps")) {
            if (request.getFragmentId().equals("patientdashboard/encountertemplate/defaultEncounterTemplate")) {
                ...
                request.setFragmentIdOverride("patientdashboard/encountertemplate/msfcoreEncounterTemplate");
