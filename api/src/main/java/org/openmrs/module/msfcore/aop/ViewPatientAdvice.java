...
public class ViewPatientAdvice implements AfterReturningAdvice {
    ...
    public void afterReturning(Object returnValue, Method method, Object[] args, Object target) throws Throwable {
        ...
        if (method.getName().equals("patientViewed") && args.length == 2 && args[0].getClass().equals(Patient.class)
            ...
            AuditLog viewPatientLog = AuditLog.builder().event(Event.VIEW_PATIENT).detail(
                            Context.getMessageSourceService().getMessage("msfcore.viewPatient") + patient.getPersonName().getFullName()
                                            + " - " + patient.getPatientIdentifier().getIdentifier()).patient(patient).user((User) args[1])
                            .build();
