...
public class AuditDaoTest extends BaseModuleContextSensitiveTest {
    ...
    public void saveAuditLog_shouldPersistLog() {
        ...
        AuditLog expected = AuditLog.builder().id(id).uuid(log.getUuid()).date(log.getDate()).event(Event.REGISTER_PATIENT).detail(
                        "registered patient#").patient(anotherPatient).user(user).location(
                        Context.getLocationService().getDefaultLocation()).provider(provider).build();
